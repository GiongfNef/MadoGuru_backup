...
if __name__ == '__main__':
    ...
    curated_vulnerabilities_json_files = ['/app/sco/sco_models/ge-sc/data/smartbug-dataset/vulnerabilities.json']
    data_type = 'buggy'
    ...
    for bug in bug_list:
        ...
        vulnerabilities_json_files = curated_vulnerabilities_json_files + [f'/app/sco/sco_models/ge-sc/data/solidifi_buggy_contracts/{bug}/vulnerabilities.json']
        print('vulnerabilities_json_files: ', vulnerabilities_json_files)
        dataset = f'/app/sco/sco_models/ge-sc/experiments/ge-sc-data/source_code/{bug}/{data_type}/'
        curated_files = [f for f in os.listdir(dataset) if f.endswith('.sol')]
        print('curated_filess:',curated_files)
        # curated_files = ['0x3f2ef511aa6e75231e4deafc7a3d2ecab3741de2.sol']
        graph_label = f'/app/sco/sco_models/ge-sc/experiments/ge-sc-data/source_code/{bug}/clean_{bug_info[bug]}_buggy_curated_0/graph_labels.json'
        ...
        graph_ckpt = f'/app/sco/models/graph_detection/nodetype/{bug}_han.pth'
        graph_graph = f'/app/sco/graphs/graph_detection/{bug}_cfg_cg_compressed_graphs.gpickle'
        ...
        node_ckpt = f'/app/sco/models/node_detection/nodetype/{bug}_han.pth'
        node_graph = f'/app/sco/graphs/node_detection/{bug}_cfg_cg_compressed_graphs.gpickle'
        ...
        data_vulnerabilities = merge_data_from_vulnerabilities_json_files(vulnerabilities_json_files)
        #print('data_vulnerabilities: ',data_vulnerabilities)
        for sc in curated_files:
            print(join(dataset, sc), ''.join(['=']*(90 - len(sc))))
            #print('dataset: ',dataset )
            #print('sc: ',sc)
            #print(join(dataset,sc))
            sm_name = 'buggy_30.sol'
            sm_path = join('/app/sco/_static', sm_name)
            copy(join(dataset, sc), sm_path)
            print("sm_path: ", sm_path)
            # sm_length = len(sm_content.split('\n'))
            cfg_graph = generate_cfg(sm_path,data_vulnerabilities)
            print('control flow graph: ',cfg_graph)
            if cfg_graph is None:
                print({'messages': 'Found an illegal CFG solidity smart contract'})
                continue
            cg_graph = generate_cg(sm_path,data_vulnerabilities)
            print('call graph: ', cg_graph)
            if cg_graph is None:
                print({'messages': 'Found an illegal CG solidity smart contract'})
                continue
            cfg_cg_graph = combine_cfg_cg(cfg_graph, cg_graph)
            print("cfg_cg_graph: ",cfg_cg_graph)
            ...
            print('original_graph_graph:', original_graph_graph)
            extra_graph = nx.disjoint_union(original_graph_graph, cfg_cg_graph)
           # print('extra_Graph:', extra_graph)
            with torch.no_grad():
                try:
            ...
            #print('targets[node_mask]: ', targets[node_mask])
            #print('node_preds[node_mask] ',  node_preds[node_mask])
